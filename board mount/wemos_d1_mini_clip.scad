...
// 
// Slight modification to extend the base so the device clip wedges inside the back mount. - github user rogerw-gh
// Make pcb offset from base a customiser variable.
// N.B. thicker pcbs will need the thickness increasing, the d1 mini clone I measured has a board thickness of 1mm, but needs 2.5mm so it fits,
// Hanging corners are not printable to size, but are so small they print reasonably well.
//
...
module edged_box()
{
    rib = 4;
    sag = .4;
    dims = [rib+.1,rib+.1,wemos_d1_pcb_thickness+sag];
    edge_off = 2;
    d = .001;
    d3 = [d,d,d];
...
module wemos_d1_mini_demo()
{
    ...
    //extension to slip inside Vindrikting case
    translate([-wemos_d1_thin_wall, -wemos_d1_thin_wall,-offset]) cube([ wemos_d1_pcb_length+9, wemos_d1_pcb_width, 1] + 2*[wemos_d1_thin_wall, wemos_d1_thin_wall,0]);
